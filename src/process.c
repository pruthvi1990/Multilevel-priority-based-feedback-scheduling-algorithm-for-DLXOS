...
int numOfProcessesInRunQueue = 0;
int temp1;
int temp2;
//Dhiraj Chaudhary//
//Function taken from bugs file
uint32 my_timer_get(){
...
//DHIRAJ CHAUDHARY begin//
  ...
  currentPCB -> p_quanta = currentPCB -> p_quanta + 1;
  Track10Quanta++;
//int temp;
...
//  printf("hello DJ The Queue with highest priority is number %d\n",temp);

  // Move the front of the queue to the end, if it is the running process.
  pcb = (PCB *)((QueueFirst (&runQueue[temp]))->object);
  currentPCB->runtime = currentPCB->runtime + (my_timer_get()-processStartTime);
...
exitsim();
  ...
  if (pcb == currentPCB)
  {
    ...
    QueueInsertLast (&runQueue[(pcb->prio)/4], &pcb->l);//pending
...
//printf("\n\n\nOMG\n\n\n\n");
	...
	//Time to insert in right queue after 4 quantum
	 QueueInsertLast (&runQueue[currentPCB->prio/4], &currentPCB->l);//pending
	}
...
for(temp1 = 0; temp1<NUM_OF_RUNQUEUE; temp1++) 
	{
...
//	pcb -> estcpu = ((double) (2/3) * pcb->estcpu) + (double)pcb-> p_nice;
 pcb->estcpu = (int)((((float)2 * 1)/((float)2 * 1 + 1)) * pcb->estcpu) + pcb->p_nice;
...
t_estcpuFinal =((((float)2 * 1)/((float)2 * 1 + 1)) * wakeup->estcpu);
t_estcpu =((((float)2 * 1)/((float)2 * 1 + 1)) * wakeup->estcpu);

for(temp1=0;temp1<(int)((my_timer_get()-wakeup->sleeptime)/1000)-1;temp1++)
{
...
//Dhiraj Chaudhary End
 ...
 //----------------------------------------------------------------------
  ...
  QueueInsertLast (&runQueue[(pcb->prio)/4], l); //all in 1.
  RestoreIntrs (intrs);
